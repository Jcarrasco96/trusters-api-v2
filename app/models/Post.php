...
class Post extends Model {
    ...
    public function index($page_size, $offset) {
        $count = $this->db->countquery("SELECT count(*) as count FROM post");
    ...
    public function owner($page_size, $offset, $user_id) {
        ...
        $count = $this->db->countquery($sqlCount);
    ...
    public function create($content, $user_id) {
        ...
        $sqlPost = sprintf("SELECT post.id, post.content, post.created_at, post.status_post, user.id as 'user_id', user.username, user.name, user.last_name, user.email, user.is_verified, user.phone, user.avatar FROM post, user WHERE post.user_id = user.id AND post.id = %u", $this->db->GetInsertID());
        $post = $this->db->uniquequery($sqlPost);
